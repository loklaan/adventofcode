...
export const solution: Solution = async ({ loadInput, debug, answer }) => {
  ...
  let gameMap = new Map<CardNumber, { copies: number; winning: number[]; own: number[] }>();
  for (let gameCardIndex = 0; gameCardIndex < gameCards.length; gameCardIndex++) {
    ...
    gameMap.set(gameCardIndex, { copies: 1, winning: winningNumbers, own: ownNumbers })
  }

  let totalPileOfCards = 0;
  for (const [gameCardIndex, { copies, winning, own }] of gameMap) {
    totalPileOfCards += copies;
    const wins = winning.filter((winningNumber) => own.includes(winningNumber)).length;
